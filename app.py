...
@app.route('/')
def index():
    return render_template('astro_.html')
...
def calculate_composite_chart_api():
    ...
    composite_method = data.get("composite_method", "direct")
    
    if not data:
        return jsonify({"error": "請求中未提供 JSON 數據"}), 400
    
        # 提供預設值 'direct'，避免 data.get 返回 None
        composite_method = data.get('composite_method', 'direct')
    try:
        optional_planets = data.get('optional_planets', [])
        c1_raw = calculate_astrology_chart(
        ...
        # 3. 根據選擇的方法，計算軸點和宮位 (分流步驟)
        # 此處 composite_raw 變數只在 if/elif 區塊內被賦值
        composite_raw = None 

        if composite_method == 'reference_plane':
            # --- 參考平面法 ---
            jd_ut_mid = (c1_raw['julian_day_ut'] + c2_raw['julian_day_ut']) / 2
            lat_mid = (c1_raw['latitude'] + c2_raw['latitude']) / 2
            lon_mid = get_midpoint(c1_raw['longitude'], c2_raw['longitude'])
            
            cusps, ascmc = swe.houses_ex(jd_ut_mid, lat_mid, lon_mid, b'P')
            
            if not isinstance(cusps, tuple) or len(cusps) < 13 or not isinstance(ascmc, tuple) or len(ascmc) < 2 or cusps[1] == 0:
                error_msg = f"無法為中點時空 (Lat: {lat_mid:.2f}) 計算宮位。通常發生在極端緯度。"
                return jsonify({"error": error_msg}), 500
            
            composite_cusps_dict = {i: cusps[i] for i in range(1, 13)}
            composite_asc_deg = ascmc[0]
            composite_mc_deg = ascmc[1]

            composite_raw = {
                "house_cusps": composite_cusps_dict,
                "planet_positions": composite_positions_raw,
            }
        ...
        # 關鍵的錯誤處理：如果 composite_method 不是已知的值，
        # composite_raw 會是 None，必須在此處攔截並返回錯誤，
        # 避免後續程式碼讀取一個不存在的變數。
        if composite_raw is None:
            return jsonify({"error": f"未知的組合盤計算方法: '{composite_method}'"}), 400

        # 4. 格式化最終輸出 (共通步驟)
        return jsonify({
            "chart_type": f"composite_{composite_method}",
...
if __name__ == '__main__':
    # 檢查星曆檔案路徑是否存在
    if not os.path.exists(EPHE_PATH_CONFIG):
        print(f"警告：找不到星曆檔案資料夾 '{EPHE_PATH_CONFIG}'。")
        print("請下載 Swisseph 星曆檔案並將其放置在正確目錄中，或設定 EPHE_PATH 環境變數。")
        print("您可以從 http://www.astro.com/ftp/swisseph/ephe/ 處下載。")
